...
Both phases. Reproduce it before diagnosing.

**Write the ticket to `docs/bugs/<slug>.md` — the actual file, with the Write
tool — before you show me anything.** Printing it in chat does not count. Then
at the phase-1 gate show me the task title, the root cause with `file:line`,
what you ruled out and why, the blast radius, and the ticket path — and wait
for my yes before touching production code.
