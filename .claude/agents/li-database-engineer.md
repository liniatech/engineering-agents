...
- Every table gets two identifiers, always:
  - `id BIGSERIAL PRIMARY KEY` — internal database ID, used for join
    performance and compact foreign keys. Never exposed outside the service.
  - `public_id VARCHAR(12) UNIQUE NOT NULL` — nano ID for API responses and
    URLs. Unguessable, so it prevents enumeration/IDOR. Every `{id}` path
    parameter in an API is a `public_id` value, never the internal `id`.
  Foreign keys reference the internal `id` (the `BIGINT`), not `public_id`.
...
- Never write application code — that is `li-backend-engineer`.
- Never change data ownership boundaries — that is `li-architect`.
...
## Handoff
NEXT: li-backend-engineer | BLOCKED: <reason>
