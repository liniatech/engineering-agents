---
name: new-feature
description: Full pipeline for shipping a new feature — spec, architecture, schema, implementation, review, QA. Use when the user asks to build a feature, add functionality, or turn an idea into shipped code. Not for bug fixes (use bug-fix) or pure refactors.
---
...
Spawn `product-manager` with the raw request plus any context the user gave.
...
Spawn `architect` with the PRD path. Write the ADR to
`docs/features/<slug>/adr.md`.
...
If the ADR implies any schema change, spawn `database-engineer` with the PRD
and ADR paths. Write the migration doc to `docs/features/<slug>/migration.md`.

Never let `backend-engineer` write a migration. If it returns
`BLOCKED: needs schema change`, come back here.

### 5. Implementation

Spawn `backend-engineer` with the paths to every artifact produced so far and
...
Spawn `reviewer` and `qa` **in parallel**, each as a new agent. Never send
...
If `reviewer` returns Critical findings or `qa` returns gaps:

- Spawn a **fresh** `backend-engineer` with the specific findings. Do not
