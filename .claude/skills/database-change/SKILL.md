...
Spawn `database-engineer` with the requirement and the current schema.

It owns this decision. `backend-engineer` may not write migrations.
...
Spawn `backend-engineer` with the migration doc. Application code must
tolerate both the pre- and post-migration schema during the deploy window.

### 7. Review

Spawn a fresh `reviewer`. Have it specifically check: lock duration, rollback
